...
public class AdvancedPathNavigate extends AbstractAdvancedPathNavigate {
    ...
    private void applyNodeEvaluatorDimensions() {
        int widthValue;
        int heightValue;
        int depthValue;
        if (overrideDefaultDimensions) {
            widthValue = Mth.floor(width + 1.0F);
            heightValue = Mth.floor(height + 1.0F);
            depthValue = Mth.floor(width + 1.0F);
        } else {
            widthValue = Mth.floor(ourEntity.getBbWidth() + 1.0F);
            heightValue = Mth.floor(ourEntity.getBbHeight() + 1.0F);
            depthValue = Mth.floor(ourEntity.getBbWidth() + 1.0F);
        }
        try {
            java.lang.reflect.Field entityWidthField = NodeEvaluator.class.getDeclaredField("entityWidth");
            java.lang.reflect.Field entityHeightField = NodeEvaluator.class.getDeclaredField("entityHeight");
            java.lang.reflect.Field entityDepthField = NodeEvaluator.class.getDeclaredField("entityDepth");
            entityWidthField.setAccessible(true);
            entityHeightField.setAccessible(true);
            entityDepthField.setAccessible(true);
            entityWidthField.setInt(nodeEvaluator, widthValue);
            entityHeightField.setInt(nodeEvaluator, heightValue);
            entityDepthField.setInt(nodeEvaluator, depthValue);
