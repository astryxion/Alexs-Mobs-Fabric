...
public class GuiCitadelPatreonConfig extends OptionsSubScreen {
    ...
    public GuiCitadelPatreonConfig(Screen parentScreenIn, Options gameSettingsIn) {
        ...
        followType = tag.getStringOr("CitadelFollowerType", "citadel");
    }
    ...
    protected void init() {
        ...
        this.addRenderableWidget(distSlider = new AbstractSliderButton(i - 150 / 2 - 25, j + 30, 150, 20, Component.translatable("citadel.gui.orbit_dist"), (rotateDist - 0.125F) / (5F - 0.125F)) {
            @Override
            protected void applyValue() {
                float value = 0.125F + (float) this.value * (5F - 0.125F);
                GuiCitadelPatreonConfig.this.setSliderValue(0, value);
            }

            @Override
            protected void updateMessage() {
                float value = 0.125F + (float) this.value * (5F - 0.125F);
                this.setMessage(Component.translatable("citadel.gui.orbit_dist").append(Component.literal(": " + roundTo(value, 3))));
            }
        });
        ...
        this.addRenderableWidget(speedSlider = new AbstractSliderButton(i - 150 / 2 - 25, j + 60, 150, 20, Component.translatable("citadel.gui.orbit_speed"), rotateSpeed / 5F) {
            @Override
            protected void applyValue() {
                float value = (float) this.value * 5F;
                GuiCitadelPatreonConfig.this.setSliderValue(1, value);
            }

            @Override
            protected void updateMessage() {
                float value = (float) this.value * 5F;
                this.setMessage(Component.translatable("citadel.gui.orbit_speed").append(Component.literal(": " + roundTo(value, 3))));
            }
        });
        ...
        this.addRenderableWidget(heightSlider = new AbstractSliderButton(i - 150 / 2 - 25, j + 90, 150, 20, Component.translatable("citadel.gui.orbit_height"), rotateHeight / 2F) {
            @Override
            protected void applyValue() {
                float value = (float) this.value * 2F;
                GuiCitadelPatreonConfig.this.setSliderValue(2, value);
            }

            @Override
            protected void updateMessage() {
                float value = (float) this.value * 2F;
                this.setMessage(Component.translatable("citadel.gui.orbit_height").append(Component.literal(": " + roundTo(value, 3))));
            }
        });
