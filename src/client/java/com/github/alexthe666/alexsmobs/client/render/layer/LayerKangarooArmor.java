...
public class LayerKangarooArmor extends RenderLayer<EntityKangaroo, ModelKangaroo> {
        ...
        if(!roo.isBaby()) {
            {
                ...
                if (itemstack.getItem() instanceof ArmorItem) {
                    ...
                    if (canEquipInSlot(itemstack, EquipmentSlot.HEAD, roo)) {
            ...
            }
            {
                ...
                if (itemstack.getItem() instanceof ArmorItem) {
                    ...
                    if (armoritem.getEquipmentSlot() == EquipmentSlot.CHEST) {
                        ...
                        int clampedLight = packedLightIn;
                        var dyedColorChest = itemstack.get(net.minecraft.core.component.DataComponents.DYED_COLOR);
                        if (dyedColorChest != null) { // Allow this for anything, not only cloth
                            int i = dyedColorChest.rgb();
    ...
    private void renderChestplate(EntityKangaroo entity, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, boolean glintIn, HumanoidModel modelIn, float red, float green, float blue, ResourceLocation armorResource, boolean notAVanillaModel) {
        ...
        float sitProgress = entity.prevSitProgress + (entity.sitProgress - entity.prevSitProgress) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false);
    ...
    /** Fabric 1.20.1: ItemStack.canEquip is Forge-only; use reflection if present, else treat as equippable when in slot (1:1). */
    private static boolean canEquipInSlot(ItemStack stack, EquipmentSlot slot, LivingEntity entity) {
        try {
            java.lang.reflect.Method m = ItemStack.class.getMethod("canEquip", EquipmentSlot.class, LivingEntity.class);
            return (Boolean) m.invoke(stack, slot, entity);
        } catch (Exception e) {
            return stack.getItem() instanceof ArmorItem ai && ai.getEquipmentSlot() == slot;
        }
    }
}
