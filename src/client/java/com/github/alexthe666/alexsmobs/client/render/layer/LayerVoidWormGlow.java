        ...
        if (isGlowing(worm) || special) {
            if(special){
                ...
                try {
                    ...
                    this.layerModel.renderToBuffer(matrixStackIn, consumer, 240, OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
                } catch (IllegalStateException ignored) {}
            }else{
                float f = getAlpha(worm);
                this.getParentModel().renderToBuffer(matrixStackIn, bufferIn.getBuffer(RenderType.eyes(texture)), 240, LivingEntityRenderer.getOverlayCoords(worm, 1.0F), ((int)(f * 255) << 24) | 0x00FFFFFF);
            }
        }
    }
