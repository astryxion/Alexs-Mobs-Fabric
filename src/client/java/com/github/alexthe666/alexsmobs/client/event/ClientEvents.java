...
public class ClientEvents {
    ...
    private static final ResourceLocation ROCKY_CHESTPLATE_TEXTURE = ResourceLocation.parse("alexsmobs:textures/armor/rocky_chestplate.png");
    ...
    public void onFogDensity(FogDensityEvent event) {
        ...
        if (Minecraft.getInstance().player.hasEffect(AMEffectRegistry.holder(AMEffectRegistry.LAVA_VISION)) && fogType == FogType.LAVA) {
            event.setNearPlaneDistance(-8.0F);
            event.setFarPlaneDistance(50.0F);
            event.setCanceled(true);
        }
        if (Minecraft.getInstance().player.hasEffect(AMEffectRegistry.holder(AMEffectRegistry.POWER_DOWN)) && fogType == FogType.NONE) {
            if (Minecraft.getInstance().player.getEffect(AMEffectRegistry.holder(AMEffectRegistry.POWER_DOWN)) != null) {
    ...
    public void onPreRenderEntity(RenderLivingEventPre event) {
        if (RockyChestplateUtil.isRockyRolling(event.getEntity())) {
            ...
            try {
                ...
                VertexConsumer vertexconsumer = ItemRenderer.getArmorFoilBuffer(event.getMultiBufferSource(), RenderType.armorCutoutNoCull(ROCKY_CHESTPLATE_TEXTURE), event.getEntity().getItemBySlot(EquipmentSlot.CHEST).hasFoil());
                ...
                ROCKY_CHESTPLATE_MODEL.renderToBuffer(event.getPoseStack(), vertexconsumer, event.getPackedLight(), OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
            } finally {
    ...
    public void onCameraSetup(CameraSetupEvent event) {
        if (Minecraft.getInstance().player.getEffect(AMEffectRegistry.holder(AMEffectRegistry.EARTHQUAKE)) != null && !Minecraft.getInstance().isPaused()) {
            int duration = Minecraft.getInstance().player.getEffect(AMEffectRegistry.holder(AMEffectRegistry.EARTHQUAKE)).getDuration();
