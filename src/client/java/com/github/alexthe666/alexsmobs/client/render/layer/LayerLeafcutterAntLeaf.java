...
public class LayerLeafcutterAntLeaf extends RenderLayer<EntityLeafcutterAnt, AdvancedEntityModel<EntityLeafcutterAnt>> {
    ...
    private static final ResourceLocation TEXTURE_0 = ResourceLocation.parse("alexsmobs:textures/entity/leafcutter_ant_leaf_0.png");
    private static final ResourceLocation TEXTURE_1 = ResourceLocation.parse("alexsmobs:textures/entity/leafcutter_ant_leaf_1.png");
    private static final ResourceLocation TEXTURE_2 = ResourceLocation.parse("alexsmobs:textures/entity/leafcutter_ant_leaf_2.png");
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityLeafcutterAnt entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn.hasLeaf() && !entitylivingbaseIn.isQueen() && this.getParentModel() instanceof ModelLeafcutterAnt) {
            ...
            int leafColor = getItemColor(new ItemStack(Items.JUNGLE_LEAVES), 0);
            ...
            this.getParentModel().renderToBuffer(matrixStackIn, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), ((int)(f*255)<<16) | ((int)(f1*255)<<8) | (int)(f2*255) | 0xFF000000);


        }
    }

    /** Fabric 1.20.1: get item tint via reflection (itemColors field on Minecraft). */
    private static int getItemColor(ItemStack stack, int tint) {
        try {
            java.lang.reflect.Field f = Minecraft.class.getDeclaredField("itemColors");
            f.setAccessible(true);
            Object itemColors = f.get(Minecraft.getInstance());
            return (Integer) itemColors.getClass().getMethod("getColor", ItemStack.class, int.class).invoke(itemColors, stack, tint);
