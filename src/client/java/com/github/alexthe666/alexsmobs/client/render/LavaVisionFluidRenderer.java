...
public class LavaVisionFluidRenderer extends LiquidBlockRenderer {
    ...
    public void tesselate(BlockAndTintGetter lightReaderIn, BlockPos posIn, VertexConsumer vertexBuilderIn, BlockState blockstateIn, FluidState fluidStateIn) {
        try {
            if (fluidStateIn.is(FluidTags.LAVA)) {
                // Fabric 1.20.1: use vanilla lava sprites and default tint (1:1 with Forge when no custom fluid type)
                TextureAtlasSprite still = Minecraft.getInstance().getTextureAtlas(TextureAtlas.LOCATION_BLOCKS).apply(ResourceLocation.parse("minecraft:block/lava_still"));
                TextureAtlasSprite flowing = Minecraft.getInstance().getTextureAtlas(TextureAtlas.LOCATION_BLOCKS).apply(ResourceLocation.parse("minecraft:block/lava_flow"));
                TextureAtlasSprite[] atextureatlassprite = new TextureAtlasSprite[]{ still, flowing };
                final int i = 0xFFFFFF;
                    ...
                    for(Direction direction : Direction.Plane.HORIZONTAL) {
                        ...
                        if (flag8 && !isFaceOccludedByNeighborVanilla(lightReaderIn, posIn, direction, Math.max(f44, f45), lightReaderIn.getBlockState(posIn.relative(direction)))) {
                            ...
                            if (atextureatlassprite[2] != null) {
                                if (lightReaderIn.getBlockState(blockpos).getFluidState().is(fluidStateIn.getType())) {
    ...
    private void vertexVanilla(VertexConsumer vertexBuilderIn, double x, double y, double z, float red, float green, float blue, float alpha, float u, float v, int packedLight) {
        vertexBuilderIn.addVertex((float) x, (float) y, (float) z).setColor(red, green, blue, alpha).setUv(u, v).setLight(packedLight).setNormal(0.0F, 1.0F, 0.0F);
    }
