...
public class RenderTendonSegment extends EntityRenderer<EntityTendonSegment, RenderTendonSegment.TendonSegmentRenderState> {
    ...
    private int getLightColor(Entity head, Vec3 vec3) {
        ...
        if (head.level().hasChunkAt(blockpos)) {
            int i = new BlockModelLighter().getLightCoords(Blocks.AIR.defaultBlockState(), (net.minecraft.client.multiplayer.ClientLevel) head.level(), blockpos);
            int j = new BlockModelLighter().getLightCoords(Blocks.AIR.defaultBlockState(), (net.minecraft.client.multiplayer.ClientLevel) head.level(), blockpos.above());
            int k = i & 255;
            int l = j & 255;
            int i1 = i >> 16 & 255;
            int j1 = j >> 16 & 255;
