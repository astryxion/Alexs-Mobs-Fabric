...
public class RockyChestplateUtil {
    ...
    public static void rollFor(LivingEntity roller, int ticks) {
        ...
        if (ticks == MAX_ROLL_TICKS) {
            tag.putInt(ROCKY_ROLL_TIMESTAMP, roller.tickCount);
        }
    ...
    public static int getRollingTicksLeft(LivingEntity entity) {
        ...
        return tag.getIntOr(ROCKY_ROLL_TICKS, 0);
    }

    public static int getRollingTimestamp(LivingEntity entity) {
        CompoundTag tag = CitadelEntityData.getOrCreateCitadelTag(entity);
        return tag.getIntOr(ROCKY_ROLL_TIMESTAMP, 0);
    }
    ...
    public static void tickRockyRolling(LivingEntity roller) {
        ...
        if (rollCounter == 0) {
            ...
            if (roller instanceof Player player && player.getPose() == Pose.SWIMMING && !player.isVisuallySwimming()) {
